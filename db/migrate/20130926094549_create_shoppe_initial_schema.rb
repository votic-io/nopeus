class CreateShoppeInitialSchema < ActiveRecord::Migration
  def up
    create_table 'shoppe_application' do |t|
      t.string  'name'
      t.string  'token'
    end
    add_index :shoppe_application, :token
  ...
  def down
    [:users, :tax_rates, :stock_level_adjustments, :settings, :products, :product_categories, :product_attributes, :orders, :order_items, :delivery_services, :delivery_service_prices, :countries, :application].each do |table|
