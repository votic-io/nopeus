...
module Shoppe
  class Setting < ActiveRecord::Base
    # Validations
    validates :key, presence: true, uniqueness: {scope: :application_id}
    ...
    before_validation do
      ...
      default_scope lambda {
        unless Thread.current[:application].nil?
          return where(application_id: Thread.current[:application].id)
        else
          return all
        end
      }
